/*
 * Copyright (C) 2025 by Bertold Van den Bergh <vandenbergh@bertold.org>
 *
...
/* Taking a pin over inherits what the band plan was doing with it, so claiming
   it to change one half does not glitch the other. */
...
/* One register write, so every pin in the mask moves at the same instant and
   the ones outside it are not disturbed. */
...
/* All four pads in one transfer, which is also the only way to catch them in
   the same instant. */
